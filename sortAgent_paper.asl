...
+!toPush(Ph): Ph==1 <- goYellowPosition;  .send(pushAgent,achieve,push); .print(" Yellow - A message should be sent to Push Agent."); .send(dropAgent,achieve,loop);  .print("A message should be sent to Push Agent OR LOW COLOR DETECTED."); -+colorDecided(false);  !samplecolor.
+!toPush(Ph): Ph==2 <- goBluePosition;    .send(pushAgent,achieve,push); .print(" Blue   - A message should be sent to Push Agent."); .send(dropAgent,achieve,loop); -+colorDecided(false); !samplecolor.      //.send([pushAgent],achieve,push). // go to samplecolor.
+!toPush(Ph): Ph==3 <- goGreenPosition;   .send(pushAgent,achieve,push); .print(" Green  - A message should be sent to Push Agent."); .send(dropAgent,achieve,loop);  -+colorDecided(false); !samplecolor.
+!toPush(Ph): Ph==4 <- goYellowPosition;  .send(pushAgent,achieve,push); .print(" Yellow - A message should be sent to Push Agent."); .send(dropAgent,achieve,loop); -+colorDecided(false); !samplecolor.
+!toPush(Ph): Ph==5 <- goYellowPosition;  .send(pushAgent,achieve,push); .print(" Red - A message should be sent to Push Agent.");    .send(dropAgent,achieve,loop);  -+colorDecided(false); !samplecolor.
+!toPush(Ph): Ph==6 <- goYellowPosition;  .send(pushAgent,achieve,push); .print(" Yellow/White - A message should be sent to Push Agent."); .send(dropAgent,achieve,loop);  -+colorDecided(false);  !samplecolor.
...
+!toBuild: buildFree(true) <-  goRedPosition; .send(buildAgent,achieve,build); -+colorDecided(false); .print("A message should be sent to Build Agent."); !samplecolor.    //.send([buildAgent],achieve,build). // go to samplecolor.
